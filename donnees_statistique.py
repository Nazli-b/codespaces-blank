...
st.set_page_config(page_title="Données Statistiques", page_icon="📈", layout="wide")

from constantes import (
    SEXE,
    MOIS,
    CATEGORIES_VEHICULES_GROUPE,
    TRAJET,
    GRAVITE,
    ATM,
    OBSM,
...
dataCaracteristiques["date"] = pd.to_datetime(
    dataCaracteristiques["an"].astype(str)
    + "-"
    + dataCaracteristiques["mois"].astype(str)
    + "-"
    + dataCaracteristiques["jour"].astype(str),
    format="%Y-%m-%d",
)
dataCaracteristiques["jour_semaine"] = dataCaracteristiques["date"].apply(
    get_day_of_week
)
...
nbAccidentByWeekday = (
    nbAccidentByDay.set_index("jour_semaine").reindex(jours_semaine_ordre).reset_index()
)
...
fig = px.pie(
    ...
    values="Num_Acc",
    names="catv",
    title="Répartition du nombre d'accidents par type de véhicule",
)
st.plotly_chart(fig)
